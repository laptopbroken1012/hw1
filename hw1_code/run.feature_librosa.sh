...
opensmile_path=/Downloads/opensmile-2.3.0/inst
...
for line in $(cat "list/all.video"); do
    ...
    x_wav,f_wav = librosa.load(audio/$line.wav)
...
python3 scripts/train_kmeans.py select.mfcc.csv $cluster_num kmeans.${cluster_num}.model || exit 1;
...
mkdir -p asrfeat
python3 scripts/create_asrfeat.py vocab list/all.video || exit 1;
